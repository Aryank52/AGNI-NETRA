        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 max-w-6xl mx-auto w-full">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filtered.map((evt) => {
                const devRatio = safeNumber(evt.features?.baseline_deviation_ratio, 2.8);
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                        <div className="text-right font-mono">
                          ...
                          <div className="text-base font-extrabold text-red-400">
                            {formatNumber(devRatio, 1)}x Normal
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs font-mono">
                      <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                        ...
                        <div className="text-red-400 font-bold text-sm">{formatFrp(maxFrpVal)}</div>
                        <div className="text-[10px] text-slate-400">Exceeds statistical facility envelope</div>
                      </div>

                      <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                        <div className="text-slate-500 text-[10px]">FACILITY CONTEXT</div>
                        <div className="text-white font-bold">{evt.facility_status || "Known"} Facility Context</div>
                        <div className="text-[10px] text-slate-400">{evt.state} Industrial Cluster</div>
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-1 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        <Link
                          ...
                          className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs flex items-center gap-1 transition-colors"
                        >
                        ...
                        >
                          <span>Inspect 7-Layer Dossier</span>
