"""
AGNI-NETRA — NASA FIRMS 2023 Full Historical Archive Standalone Streaming Importer
Offline, batched, idempotent ingestion for PostgreSQL 16.15 + PostGIS 3.4.2.

Supported Standard Science Products:
  1. NOAA-20 VIIRS 375m (VJ114IMGTDL, Collection 2) - DL_FIRE_J1V-C2_795858.zip
  2. Suomi-NPP VIIRS 375m (VNP14IMGTDL, Collection 2) - DL_FIRE_SV-C2_795859.zip
  3. MODIS Terra/Aqua 1km (MCD14DL, Collection 6.1) - DL_FIRE_M-C61_795857.zip
...
Features:
  ...
  - Strict preservation of 2022 official (1,274,383), 2022 pilot (210,000), and 2026 baseline (1,771,110) records
  - Generates authoritative archive manifest (archive_manifest_2023.json)
"""
...
CANDIDATE_DIRS = [
    r"E:\PROJECTS\AGNI-NETRA(DATABASE)\FIRMS\HISTORICAL\2023\full",
    r"E:\AGNI-NETRA-DATA\FIRMS\HISTORICAL\2023\full"
]
...
def import_2023_full_archives():
    print("=" * 85, flush=True)
    print(f"  AGNI-NETRA — PHASE 5C: NASA FIRMS {TARGET_YEAR} FULL ARCHIVE INGESTION (HIGH-SPEED)", flush=True)
    print("=" * 85, flush=True)
    ...
    for arc in archives:
        print(f"  - {arc['zip_name']} ({arc['file_size'] / (1024*1024):.2f} MB) | Product: {arc['product']} | Sensor: {arc['sensor']}", flush=True)
    ...
    with engine.raw_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(f"SELECT COUNT(*) FROM thermal_detections WHERE EXTRACT(YEAR FROM acq_timestamp) = {TARGET_YEAR};")
    ...
    print(f"  {TARGET_YEAR} Detections (Current): {det_target_before:,}\n", flush=True)
            ...
            with zf.open(csv_name) as cf:
                ...
                for row in reader:
                    ...
                    det_tuple = (
                        ...
                        None,
                        metadata_json,
                        False  # is_demo = False (Official 2023 Full Archive)
                    )
    ...
    print(f"Real Official 2023 (DB) : {det_2023_real_after:,} (is_demo=False)", flush=True)
